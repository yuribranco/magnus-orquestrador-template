...
### Passo 1 — Campanha + contexto + oferta
Identifique a **campanha** (ex: `lancamento-q3`): se nova, crie `operacao/<slug>/` + `BRIEFING.md`; se existe, leia o `BRIEFING.md` dela. Leia `contexto/` (EMPRESA, VOZ, DESIGN, ativos — brand compartilhado). Pergunte UMA por vez: oferta/produto · público (stage of awareness) · promessa principal · preço/bônus/garantia · CTA principal · que prova existe (depoimentos?).
...
Produza um `index.html` standalone em `operacao/<campanha>/landing/`:
